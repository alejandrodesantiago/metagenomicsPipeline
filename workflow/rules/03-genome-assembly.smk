...
#        "../envs/megahit.yaml"
    run:
        R1_paired_list = ",".join(s.split(map(str,input.R1)))
        R2_paired_list = ",".join(s.split(map(str,input.R2)))
        R1_unpaired_list = ",".join(s.split(map(str,input.unpaired_R1)))
        R2_unpaired_list = ",".join(s.split(map(str,input.unpaired_R2)))
