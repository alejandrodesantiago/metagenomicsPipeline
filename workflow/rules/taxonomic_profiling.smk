rule flash:
    ...
    output:
        merged=scratch_dir + "01-analysis/07-flash/{sample}_merged.fastq.gz"
    params:
    conda:
        ".../envs/flash.yaml"
    shell:
...
rule metaphlan:
    input:
        reads=expand(scratch_dir + "01-analysis/07-flash/{sample}_merged.fastq.gz", sample=SAMPLES)
    output:
        profile=scratch + "01-analysis/08-metaphlan/{sample}_taxonomy_profile.txt"
    params:
    conda:
        ".../envs/metaphlan.yaml"
    shell:
